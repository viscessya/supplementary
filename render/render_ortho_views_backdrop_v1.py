# -*- coding: utf-8 -*-
"""Gray-background variant of render/render_ortho_views.py: identical except that the world background
is neutral gray (0.5) instead of white. There is no alpha compositing and no change to framing.

Known confound, recorded before the first render: in Blender Cycles the world background is also an
ambient light source, so the gray background slightly reduces the ambient light on the object. It is
disclosed rather than removed (manuscript Section 3.3).

Usage: blender -b -P render_ortho_views_backdrop_v1.py -- --input model.stl --out output_dir --elev 20 --margin 1.30
...
print(f"\nSelesai. 4 view (latar abu-abu 0,5) tersimpan di: {outdir}")
